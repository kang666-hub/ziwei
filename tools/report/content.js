...
function buildPrompt(chart) {
  ...
  const yearlyMutagenText = yearly.mutagen.map((m) => `${m.star}${m.hua}`).join('、') || '無';
...
B. 星曜位置描述：全文任何提到某顆星曜所在宮位的句子，都只能依照上方「星曜–宮位對照表」或「十二流月命宮對照表」括號內標註的宮位陳述，不可依命理知識或本命特性自行推論、杜撰任何表中未列出的星曜位置；同一顆星曜在全文中的宮位描述必須前後一致，不得矛盾。
C. 文字規範：全文必須使用繁體中文（正體字），嚴禁出現任何簡體字。
D. 化曜聚合描述：流月四化的每顆星實際所在宮位已在「十二流月命宮對照表」括號內標明，只有當兩顆以上化曜標註的宮位確實相同時，才可以用「齊聚」「交會」「同入」等聚合語句描述該宮；若化曜分散在不同宮位，只能逐一單獨陳述，例如「當月OO化祿（星在OO宮）遙應」，或分開敘述各宮各自受到的四化影響，嚴禁虛構化曜聚集在同一宮位。
